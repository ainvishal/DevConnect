...
from .models import Post, PostInteraction, UserFollower
...
def user_profile(request, user_id):
    try:
        user = User.objects.get(id=user_id)
        user_follower, _ = UserFollower.objects.get_or_create(user=user)
        request_user_follower, _ = UserFollower.objects.get_or_create(user=request.user)

        is_following = request.user.userfollower.following.filter(id=user.id).exists()

        data = {
            'username': user.username,
            'email': user.email,
            'followers_count': user_follower.followers.count(),
            'following_count': user_follower.following.count(),
            'is_following': is_following,
        }
        return Response(data)
...
def follow_unfollow(request, user_id):
    try:
        ...
        user_follower, _ = UserFollower.objects.get_or_create(user=request.user)

        if user_to_follow in user_follower.following.all():
            user_follower.following.remove(user_to_follow)
            user_to_follow.userfollower.followers.remove(request.user)
            return Response({'message': 'Unfollowed successfully'})
        else:
            user_follower.following.add(user_to_follow)
            user_to_follow.userfollower.followers.add(request.user)
            return Response({'message': 'Followed successfully'})
